...
export class TimeoutOptionTests {
   ...
   public errorThrownIfTimeoutValueIsInvalid(invalidTimeout: string) {
      Expect(() => new AlsatianCliOptions([ "--timeout", invalidTimeout ]))
        .toThrowError(InvalidTimeoutValueError, `Invalid timeout value "${invalidTimeout}" given.`);
   }
   ...
   public duplicateTimeoutArgumentsThrowsError(...inputArguments: Array<string>) {
      Expect(() => new AlsatianCliOptions([ inputArguments[0], "1", inputArguments[1], "42" ]))
        .toThrowError(DuplicateCliArgumentError, "Duplicate \"timeout\" arguments were provided.");
   }

   @TestCase("-t")
   @TestCase("--timeout")
   public missingTimeoutPeriodThrowsError(timeoutArgument: string) {
      Expect(() => new AlsatianCliOptions([ timeoutArgument ]))
        .toThrowError(MissingArgumentValueError, "Argument \"timeout\" requires a value.");
   }
}
