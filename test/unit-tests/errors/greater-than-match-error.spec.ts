...
export class GreaterThanMatchErrorTests {
  ...
  public shouldBeGreaterThanMessage(actualValue: number, lowerLimit: number) {
    ...
    Expect(error.message).toBe(
      "Expected " + actualValue + " to be greater than " + lowerLimit + "."
    );
  }
  ...
  @TestCase(2, 2)
  @TestCase(42, 1)
  public shouldNotBeGreaterThanMessage(
    actualValue: number,
    lowerLimit: number
  ) {
    ...
    Expect(error.message).toBe(
      "Expected " + actualValue + " not to be greater than " + lowerLimit + "."
    );
  }
  ...
  public shouldSetActualValueToGivenValue(actualValue: number) {
    Expect(new GreaterThanMatchError(actualValue, -42, true).actual).toBe(
      actualValue
    );
  }
  ...
  public shouldSetExpectedValueToGreaterThanLowerLimit(lowerLimit: number) {
    Expect(new GreaterThanMatchError(512, lowerLimit, true).expected).toBe(
  ...
  public shouldSetExpectedValueToNotGreaterhanLowerLimit(lowerLimit: number) {
    const greaterThanMatchError = new GreaterThanMatchError(
      512,
      lowerLimit,
      false
    );

    Expect(greaterThanMatchError.expected).toBe(
