...
export class ToThrowErrorTests {
  ...
  public differentErrorThrownWhenNoneExpectedPasses() {
    ...
    Expect(() =>
      Expect(throwWrongErrorFunction).not.toThrowError(Error, "error message")
    ).not.toThrow();
  }
  ...
  public differentMessageErrorThrownWhenNoneExpectedPasses() {
    ...
    Expect(() =>
      Expect(throwWrongMessageFunction).not.toThrowError(Error, "error message")
    ).not.toThrow();
  }
  ...
  public checkingWhetherNonFunctionThrowsErrorShouldThrow(actualValue: any) {
    ...
    Expect(() => EXPECT.toThrowError(Error, "")).toThrowError(
      TypeError,
  ...
  @TestCase([])
  @TestCase(["an", "array"])
  public checkingWhetherNonFunctionDoesNotThrowErrorShouldThrow(
    actualValue: any
  ) {
    const EXPECT = Expect(() => {});
    (EXPECT as any)._actualValue = actualValue;

    Expect(() => EXPECT.not.toThrowError(Error, "")).toThrowError(
      TypeError,
      "toThrowError requires value passed in to Expect to be a function."
    );
  }
