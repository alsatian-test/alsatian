...
export class ErrorMatchError extends MatchError {
  public constructor(
    ...
    this._actual =
      `${
        actualError ? (actualError.constructor as INameable).name + " " : ""
      }` +
    ...
    this._expected =
      `${
        expectedErrorType ? (expectedErrorType as INameable).name + " " : ""
      }` +
  ...
  private _setWrongMessageAndTypeMessage(
    ...
    this.message =
      `Expected an error with message "${expectedErrorMessage}" ` +
      `and type ${(expectedErrorType as INameable).name} to ${
        !shouldMatch ? "not " : ""
      }` +
  ...
  private _setWrongTypeMessage(
    ...
    this.message =
      ...
      `but ${
        shouldMatch
          ? (actualError as INameable).name + " was thrown instead"
          : "it was"
      }.`;
  }
}
