...
export class FunctionSpyMatcherTests {
	...
	@TestCase(-2)
	@TestCase(-42)
	public negativeOrZeroAnythingButValueThrowsError(
		expectedCallCount: number
	) {
		const some = {
			function: () => {}
		...
		Expect(
			() => spyMatcher.anythingBut(expectedCallCount).times
		).toThrowError(
			TypeError,
	...
	@TestCase(-2)
	@TestCase(-42)
	public negativeOrZeroGreaterThanValueThrowsError(
		expectedCallCount: number
	) {
		const some = {
			function: () => {}
		...
		Expect(
			() => spyMatcher.greaterThan(expectedCallCount).times
