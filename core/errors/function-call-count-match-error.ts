...
export class FunctionCallCountMatchError extends MatchError {
  ...
  public constructor(actualValue: FunctionSpy, shouldMatch: boolean, expectedCallCount: number, args?: Array<any>) {
    ...
    super(`function was ${shouldMatch && !(args && actualValue.calls.length) ? "not " : ""}called${args && actualValue.calls.length ? " with " + actualValue.calls.map(call => JSON.stringify(call.args)).join(", ") : ""} ${expectedCallCount} times.`,
          `function ${!shouldMatch ? "not " : ""}to be called${args ? " with " + FunctionCallCountMatchError._stringifyArguments(args) : ""}.`,
          `Expected function ${!shouldMatch ? "not " : ""}to be called${args ? " with " + FunctionCallCountMatchError._stringifyArguments(args) : ""} ${expectedCallCount} times.`);
  }
