...
export class FunctionCallCountMatchError extends MatchError {
  ...
  public constructor(actualValue: FunctionSpy, shouldMatch: boolean, expectedCallCount: number, args?: Array<any>) {
    ...
    super(`function was ${!shouldMatch ? "not " : ""}called${args && actualValue.calls.length ? " with " + actualValue.calls.map(call => JSON.stringify(call.args)).join(", ") : ""} ${actualValue.calls.length} time${actualValue.calls.length=== 1 ? "" : "s"}.`,
