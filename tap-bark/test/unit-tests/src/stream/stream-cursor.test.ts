...
export default class StreamWritingTests {
  ...
  public clearLineCalledWithStreamAndPosition0(
    expectedStream: NodeJS.WritableStream
  ) {
    ...
    (Readline.clearLine as any).restore();
  }
  ...
  public moveCursorActsOnInnerStream(expectedStream: NodeJS.WritableStream) {
    ...
    (Readline.moveCursor as any).restore();
  }
  ...
  public moveCursorSetsCorrectXPosition(expectedXPosition: number) {
    ...
    Expect(Readline.moveCursor).toHaveBeenCalledWith(
      stream.stream,
      expectedXPosition,
      0
    );
  ...
  public cursorToActsOnInnerStream(expectedStream: NodeJS.WritableStream) {
    ...
    (Readline.cursorTo as any).restore();
  }
  ...
  public cursorToSetsCorrectXPosition(expectedXPosition: number) {
    ...
    Expect(Readline.cursorTo).toHaveBeenCalledWith(
      stream.stream,
      expectedXPosition,
      0
    );
